...
public static class Settings {
	...
	private static List<SettingDef> BuildGlobal() => [
		...
		new("Rep4RepPauseDays", "Hold commenting for", SecRep4RepAccount, SettingKind.Int,
			"Stop commenting completely for this many days, on every account, then carry on by itself. Set it and forget it - the countdown survives a restart and clears itself when it runs out, so you do not have to remember to switch anything back on. 0 means nothing is being held. The daily cap and the schedule are untouched; this simply sits the whole thing out.",
			Min: 0, Max: 30),
