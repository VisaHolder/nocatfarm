...
new("PluginsEnabled", "Load plugins", SecDashboard, SettingKind.Bool,
		...
		new("Debug", "Debug detail", SecLogging, SettingKind.Bool,
			"Write a lot more detail to the log FILE - every web request, every re-check, the lot. On by default, because it never reaches the window or the console: it costs you nothing on screen and means that when something does go wrong the answer is already in the file, rather than needing the fault reproduced with this switched on. The dashboard's Log tab can show it too, behind its own DEBUG filter.",
			Advanced: true),
