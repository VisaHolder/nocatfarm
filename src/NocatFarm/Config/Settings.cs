...
public static class Settings {
	...
	private static List<SettingDef> BuildBot() => [
		...
		new("RefundHoldDays", "...for this many days", SecCards, SettingKind.Int,
			...
			Advanced: true, Min: 1, Max: 90),
		new("LegitFarming", "Farm in sittings, not flat out", SecCards, SettingKind.Bool,
			"Farm the way somebody plays: a few sittings a day of believable length, with gaps between them, starting and finishing at different times, longer at the weekend. A farmer that runs flat out puts hours on the account in a straight line, through the night, every night - which is the most obvious thing on it. This works with or without human mode, so an account that only farms can still look like one that plays. It changes WHEN it farms, never what."),
		new("LegitFarmHoursPerDay", "Hours a day to farm", SecCards, SettingKind.Int,
			"Roughly how much of the day the sittings add up to. Rolled fresh every day within a spread of this, and longer at the weekend, so no two days are the same shape. Only used when farming in sittings is on.",
			Min: 1, Max: 20),
