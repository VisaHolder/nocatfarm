...
new("PluginsEnabled", "Load plugins", SecDashboard, SettingKind.Bool,
		...
		new("Debug", "Debug detail", SecLogging, SettingKind.Bool,
			"Write a lot more detail to the log FILE - every web request, every re-check, the lot. It never appears in the console window, so turning it on costs you nothing on screen; it is there for when something is wrong and you want the whole story afterwards. The dashboard's Log tab can show it too, behind its own DEBUG filter.",
			Advanced: true),
