...
public static class Settings {
	...
	private static List<SettingDef> BuildBot() => [
		...
		new("AchievementMaxCompletionPct", "Finish no more than", SecAchievements, SettingKind.Int,
			"How much of any one game gets completed, as a percentage. 0 leaves each game to its own tuned ceiling - a 3-hour puzzle game and a 500-hour grind should not stop at the same figure. Set a number and it wins outright, raising a game's ceiling as readily as lowering it, for when an account has genuinely played something to death and should be allowed to finish it. It never goes above 95: a 100%%-completed game on an idled account is itself the giveaway.",
			Min: 0, Max: 95),
