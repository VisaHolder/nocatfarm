...
public static class Settings {
	...
	private static List<SettingDef> BuildGlobal() => [
		...
		new("OpenDashboardAfterAdd", "Open the dashboard after adding an account", SecDashboard, SettingKind.Bool,
			"Open the dashboard in your browser once a new account has been added, so you can set it up. A brand new account does nothing at all until it is told what to play, and the app window has no form for that - only the dashboard does. Turning this off means you are on your own to remember.", Advanced: true),
		new("OpenBrowserOnStart", "Open the browser on start", SecDashboard, SettingKind.Bool,
			"Open the dashboard in your browser automatically when nocat.farm starts.", Advanced: true),
		...
		new("StartMinimized", "Start hidden", SecBackground, SettingKind.Bool,
			"Start with the console window hidden, straight to the tray. Double-click the tray icon to bring it back.", Advanced: true),
		...
		new("KeepAwake", "Keep this PC awake", SecBackground, SettingKind.Bool,
			"Stop Windows going to sleep while accounts are running. The screen can still switch off - only sleep is blocked.", Advanced: true),
		...
		new("NotifyEarnings", "Pop up when you earn", SecBackground, SettingKind.Bool,
			"Pop up when a trading card drops or a rep4rep comment is credited.", Advanced: true),
		new("NotifySocial", "Pop up for comments", SecBackground, SettingKind.Bool,
			"Pop up when somebody comments on one of your profiles.", Advanced: true),
		new("NotifyProblems", "Pop up for problems", SecBackground, SettingKind.Bool,
			"Pop up when an account genuinely needs you: a Steam Guard code, a failed login, or a comment ban.", Advanced: true),
		...
		new("Rep4RepPauseHours", "Hold commenting for (hours)", SecRep4RepAccount, SettingKind.Int,
			"Stop commenting completely for this many hours, on every account, then carry on by itself. 24 is a day, 48 is two - and anything in between works, so a 27-hour pause is just 27. Set it and forget it: the countdown survives a restart, is measured from when you set it rather than from the last time you saved anything, and clears itself when it runs out. 0 means nothing is being held. The daily cap and the schedule are untouched; this simply sits the whole thing out, and every account comes back at a clean 0/10.",
			Min: 0, Max: 720),
		new("Rep4RepAutoAddProfiles", "Register accounts automatically", SecRep4RepAccount, SettingKind.Bool,
			"Register each Steam account with rep4rep the first time it logs in, so you don't have to add them by hand on the website.", Advanced: true),
...
new("PluginsEnabled", "Load plugins", SecDashboard, SettingKind.Bool,
		...
		new("Debug", "Show debug detail on screen", SecLogging, SettingKind.Bool,
			"Debug detail - every web request, every re-check, the lot - is ALWAYS written to the log file, whatever this is set to. That is the point of a log: when something goes wrong the answer is already recorded instead of needing the fault reproduced with a switch flipped. This only decides whether those lines are also shown in the window and the console, which is off by default because they scroll the handful of lines you actually wanted off the top."),
	...
	private static List<SettingDef> BuildBot() => [
		...
		new("StartPaused", "Start paused", SecAccount, SettingKind.Bool,
			"Log in but don't play, farm or comment until you press Resume. For an account you want online and doing nothing.", Advanced: true),
		...
		new("AchievementMaxCompletionPct", "Finish no more than", SecAchievements, SettingKind.Int,
			"How much of any one game will ever be completed, as a percentage. This is the only ceiling there is - it applies to every game the same way, so what you set is what you get. 90 leaves a handful unearned in everything, which is what almost every real library looks like; a game sitting at exactly 100%% on an account that idles is the thing somebody notices. A deliberate 'grind' ignores it, because that is the explicit finish-this-game path.",
			Min: 1, Max: 100),
		new("AchievementIncludeMainGame", "Earn them in the main game too", SecAchievements, SettingKind.Bool,
			"Whether human mode's main game earns achievements like every other game. On, and it should be: the main game is where the hours are, so it's the least surprising place on the account to see them. Turning this off leaves it completely alone, which means an account with hundreds of hours in its headline game shows nothing for them - usually the stranger of the two. Either way the rarity gate and the completion ceiling still apply."),
		new("AchievementBoost", "Achievement boost", SecAchievements, SettingKind.Choice,
			"Auto-hunt achievements across several games without starting each grind by hand. OFF by default; most accounts won't use it. \"Games you pick\" works through the list below; \"all single-player\" finds them itself - every single-player game in the library that has achievements, is a game (never DLC, demos, soundtracks or tools) and that people actually play. Either way it takes one game at a time, plays it like a normal grind (easiest-first, at your Achievement pace, only what the hours make reachable), then rotates. On a human-mode account it stays weighted-first: a session here and there between long stretches of the normal schedule, never while asleep.", Choices: "0 off | 1 games you pick | 2 all single-player"),
		...
		new("AutoReplyOncePerDay", "Only reply once a day", SecSocial, SettingKind.Bool,
			"Send that reply at most once a day to any one person. Without this, anybody who messages twice gets the identical line twice and it is instantly obvious what they are talking to.", Advanced: true),
		...
		new("PauseWhenYouPlay", "Stand down when you play", SecCourtesy, SettingKind.Bool,
			"Stop playing the second you launch a game on this account yourself, so nocat.farm never fights your own Steam client for the session.", Advanced: true),
